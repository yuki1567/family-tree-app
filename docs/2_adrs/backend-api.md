...
ドメイン別に `routes/` 配下へ1ファイルずつ配置し、`app.ts` でまとめる。controller 層は設けず、route ファイルが HTTP の関心（リクエスト解析・サービス呼び出し・レスポンス返却）を担う。
...
- controller 層を設ける: ハンドラが薄い（validate → service call）ためファイルが増えるだけで抽象化の利点がない。

### 4. RPC 型共有 — Vite path alias で `AppType` をフロントに配線

フロントから Hono RPC クライアント（`hono/client`）を型付きで使うため、`AppType` をフロントに import できるようにする。

- Vite の `resolve.alias` と tsconfig の `paths` に `@backend` エイリアスを設定し、フロントから `import type { AppType } from '@backend/app'` で参照する
- `import type` のみのため、バックエンドのコードは実行時に一切バンドルされない
- `noUndeclaredDependencies`（Biome ルール）が path alias を未宣言パッケージとして検知するため、フロントエンドのフォルダ構造確定後に `biome.json` の `overrides` で対象ディレクトリのみ無効化する

**却下した代替案**
- workspace 依存（`frontend` が `backend` を `devDependencies` に宣言）: import 文はシンプルになるが、フロントがバックエンドを依存として持つ形になる。

### 5. Drizzle ORM / DB 接続・スキーマ・マイグレーション

**ディレクトリ構成**

スキーマ定義・DB クライアント・生成済みマイグレーション SQL をすべて `src/db/` 配下に統一する。

**マイグレーション管理**

`drizzle-kit generate` でスキーマ差分から SQL を生成し、`drizzle-kit migrate` で適用する。マイグレーションファイルが残るため本番への適用も同じ SQL で行える。`push`（差分を直接 DB に適用）はファイルが残らず本番運用に不向きなため不採用。
